...
export const wholeTaxonomyGroupsHandler: Handler<
  ReadonlyArray<TaxonomySyncModel>
> = makeWholeObjectsHandler();

type FlattenedSyncTaxonomy = Replace<TaxonomySyncModel, { terms: [] }> &
  Readonly<{ position: ReadonlyArray<string> }>;
      ...
      makeAdjustOperationHandler(
        ...
        makeObjectHandler({
          ...
          position: (sourcePos, targetPos) => {
            if (
              sourcePos.length === targetPos.length &&
              zip(sourcePos, targetPos).every(([a, b]) => a === b)
            ) {
              return [];
            }
              ...
              ? [
                  {
                    ...
                    before: {
                      codename:
                        topLevelTermCodename ??
                        throwError(noTopLevelWhenMovigToTopErrMsg),
                    },
                  },
                ]
              : [
...
const stripPositionFromPath = (op: PatchOperation): PatchOperation => ({
  ...op,
  path: op.path.startsWith("/position")
    ? op.path.slice("/position".length)
    : op.path,
});

const addPositionBackIntoPath =
  (
    sourceTerms: ReadonlyArray<FlattenedSyncTaxonomy>,
    targetTerms: ReadonlyArray<FlattenedSyncTaxonomy>,
  ) =>
...
const removePositionProp = (op: PatchOperation): PatchOperation => ({
  ...
  op.value !== null &&
  typeof op.value === "object" &&
  "position" in op.value
    ? { value: omit(op.value, ["position"]) }
    : {}),
  ...("oldValue" in op &&
  op.oldValue !== null &&
  typeof op.oldValue === "object" &&
  "position" in op.oldValue
...
const flattenTaxonomy = (
  ...
  ...taxonomy.terms.flatMap((t) =>
    flattenTaxonomy(t, [...position, taxonomy.codename]),
  ),
];
